/*
 * unlock_v2.c — NVIDIA CMP 90HX (GA102) full unlock — UEFI application
 *
 ...
 *
 * Target hardware : NVIDIA CMP 90HX — GA102 die, 10 GB GDDR6X, PCI 10de:220d,
 ...
 *   fwsec_ga102.bin (+_sig)        FWSEC ucode + signature, extracted VBIOS
               ...
               UINT64 sigPhys, UINT64 fbSize, UINT64 blPhys, UINT64 blSize)
{
    ...
    m->gspFwHeapSize   = 0x7F00000ULL;   /* kgspGetFwHeapSize для CMP90HX/10GB */
...
find_cmp90hx(void)
{
            ...
            for (Dev = 0; Dev < 32; Dev++) {
                ...
                for (Fn = 0; Fn < MaxFn; Fn++) {
                    ...
                    Status = uefi_call_wrapper(gRb->Pci.Read, 5, gRb,
                        EfiPciIoWidthUint32, AF, 1, &Id);
                    if (EFI_ERROR(Status) || Id == 0xFFFFFFFF) continue;
                    if ((Id & 0xFFFF) == 0x10DE && ((Id >> 16) & 0xFFFF) == 0x220D) {
                        gBus = Bus; gDev = Dev; gFn = Fn;
                        Print(L"PCI: CMP90HX найден (bus=%d dev=%d fn=%d)\n",
    ...
    FreePool(RbHandles);
    Print(L"PCI: CMP90HX НЕ найден (scanned %d bridges)\n", (INTN)RbCount);
    return EFI_NOT_FOUND;
}
...
# endif
#else
    Print(L"\n=== CMP90HX Unlock v2.101 (multipass + GFX/SS verify) ===\n");
#endif
